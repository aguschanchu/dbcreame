...
def convert(stlpath, name, rotate=False):
    ...
    stl_path = os.path.join(settings.BASE_DIR, 'tmp', name.split('/')[-1].split('.')[0] + '.stl')
    with open(stl_path, 'wb') as f:
        f.write(trimesh.exchange.stl.export_stl(mesh))
    # Convertimos el stl a obj usando assimp
    obj_path = os.path.join(settings.BASE_DIR, 'tmp', name.split('/')[-1].split('.')[0] + '.obj')
    ...
    # Convertimos el sfb
    args = [os.path.join(settings.BASE_DIR, 'lib/sceneform_sdk/linux/converter'), '-d', '--mat',
            os.path.join(settings.BASE_DIR, 'lib/sceneform_sdk/default_materials/obj_material.sfm'),
            '--outdir', os.path.join(settings.BASE_DIR, 'tmp'), obj_path]
    proc = subprocess.run(args, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    return os.path.join(settings.BASE_DIR, 'tmp', name.split('/')[-1].split('.')[0] + '.sfb')
...
def combine_stls_files(objeto):
    ...
    args = [os.path.join(settings.BASE_DIR, 'lib/simarrange/simarrange')]
    for archivo_stl in objeto.files.all():
        args.append(archivo_stl.model.get_model_path())
    # Buscamos build_plate_size minimo
    while not arrange_completed:
        args_d = args.copy() + ['--dryrun', '-x', str(build_plate_size), '-y', str(build_plate_size), '--spacing',
                                str(10)]
        ...
        for line in proc.stdout.splitlines():
            ...
            dir = os.path.join(settings.BASE_DIR, 'tmp', ''.join(random.choices(string.ascii_uppercase + string.digits, k=8)))
            args_d = args.copy() + ['-x', str(build_plate_size), '-y', str(build_plate_size), '--spacing', str(10),
                                    '--outputdir', dir + '/']
            os.mkdir(dir)
            proc = subprocess.run(args_d, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                  cwd=dir + '/')
...
def combine_stl_with_correct_coordinates(objeto):
    ...
    STL cominado
    '''
    meshes = [trimesh.load_mesh(a.model.get_model_path()) for a in objeto.object.files.all()]
